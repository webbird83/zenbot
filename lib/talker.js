...
function initVars() {
  var rwOpts =
  {
    ...
    "sell_stop_pct": { "et ldValue": oo.sell_stop_pct, "newValue": no.sell_stop_pct },
...
function writeOpts (message) {
  ...
  Object.keys(message).forEach(function(key){
    ...
    if (n(fun.oldvalue) >= 0) {
      ...
      ov = '  =  ' + fun.oldValue 
    if (fun.newValue === undefined) {
      nv = 'undef'
    }
    else nv = fun.newValue
    opts += ' '.repeat(24 - key.length) + key + ov
    + '  -->  ' + nv + '\n'
//  console.log(key)
//  console.log(opts)
  })
